...
export class InitiateWorkflowComponent {
  ...
  getObject(worksheet: XLSX.WorkSheet, rows: any) {
    let finalObj = {};
  ...
  getCompleteRRDump(worksheet: XLSX.WorkSheet, units: number) {
    const complete_data:any[] = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: false});
    let formattedData:any[] = [];
    const range = units + 20;
    let count =2;
    for(let i = 2; i < range; i++){
      count++;
      if(complete_data[i][12]){
        const obj = {
          name: complete_data[i][6],
          market: parseInt(complete_data[i][7].replace(/,/g, ''), 10),
          actual: parseInt(complete_data[i][8].replace(/,/g, ''), 10),
          movein: new Date(this.getFormattedDate(complete_data[i][11])),
          leaseFrom: new Date(this.getFormattedDate(complete_data[i][12]))
        }
  ...
  async rrDumpInput(path: string, event: any) {
    ...
    this.communityRentRole = this.getCompleteRRDump(worksheet, units.total);
    this.communityUnits = {
